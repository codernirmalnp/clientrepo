...
export default class AuthController{
    ...
    public async signin (req:express.Request,res:express.Response){
        try{
            ...
            if(auth_type=="email"){
                return  await this.signInUseCase.execute(name, email, password, auth_type).then((user:usr)=>{
                    const accessToken=this.tokenService.encode({id:user.id,name:user.name},true,false)
                    const refreshToken=this.tokenService.encode({id:user.id,name:user.name},false,true)
                    this.tokenStore.save(refreshToken)
                    res.cookie('access_token',accessToken)
                    res.cookie('refresh_token',refreshToken)
                    user.password=undefined
                   return res.status(200).json({status:true,msg:"Login SuccesFull",data:user})
                   
                  }).catch((err)=>res.status(400).json({status:false,err:err}))
